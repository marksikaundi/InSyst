...
export default function AuditForm() {
  ...
  const [auditResults, setAuditResults] = useState<Record<
    string,
    | string
    | number
    | boolean
    | string[]
    | { [key: string]: number }
    | { tagName: string; id: string; className: string; isClickable: boolean }[]
  > | null>(null);
    ...
    <div className="space-y-8">
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <Button
          type="submit"
          disabled={isLoading}
          className="w-full bg-black hover:bg-black text-white font-bold py-3 px-4 rounded-lg transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50"
        >
          {isLoading ? (
            <>
